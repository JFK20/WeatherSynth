...
namespace WeatherModel.Climate
{
    ...
    public sealed class ScaledBeta
    {
        ...
        {
            if (!(alpha > 0.0)) throw new ArgumentOutOfRangeException(nameof(alpha), alpha, "Must be positive.");
            if (!(beta > 0.0)) throw new ArgumentOutOfRangeException(nameof(beta), beta, "Must be positive.");
        ...
        public static ScaledBeta FitByMoments(IEnumerable<double> values, double scale)
        {
            ...
            foreach (double value in values)
            {
                ...
                if (value < 0.0 || value > scale)
                    throw new ArgumentException(
                        ...
                        "a Beta cannot represent observations beyond its own bounds.", nameof(values));
            ...
            if (n < 2)
                throw new ArgumentException("Need at least two observations to fit.", nameof(values));
            ...
            if (v <= 0.0)
                throw new ArgumentException("Sample has no variance; a Beta cannot be fitted.", nameof(values));

            double maxVariance = m * (1.0 - m);
            if (v >= maxVariance)
                throw new ArgumentException(
                    $"Sample is too dispersed for any Beta: variance {v:F5} is at or above the " +
                    $"family's limit {maxVariance:F5} for mean {m:F4}.", nameof(values));
        ...
        public double Density(double x)
        {
            ...
            if (u <= 0.0) return Alpha < 1.0 ? double.PositiveInfinity : Alpha > 1.0 ? 0.0 : Beta / Scale;
            if (u >= 1.0) return Beta < 1.0 ? double.PositiveInfinity : Beta > 1.0 ? 0.0 : Alpha / Scale;
        ...
        private static double IncompleteBetaFraction(double x, double a, double b)
        {
            ...
            for (int m = 1; m <= 300; m++)
            {
                Advance(m * (b - m) * x / ((a + 2.0 * m - 1.0) * (a + 2.0 * m)),
                    ref c, ref d, ref result);
                Advance(-(a + m) * (a + b + m) * x / ((a + 2.0 * m) * (a + 2.0 * m + 1.0)),
                    ref c, ref d, ref result);
