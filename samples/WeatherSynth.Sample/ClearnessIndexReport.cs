...
public static class ClearnessIndexReport
{
    public static void Run(IReadOnlyList<DwdSolarDay> days, DwdStation station)
    {
        ...
        Console.WriteLine(
            $"  {complete.Count:N0} complete, less {complete.Count - usable.Count} "
    ...
    private static void Histogram(IReadOnlyList<DailyClearness> series)
    {
        ...
        if (counts[bins] > 0)
            Console.WriteLine(
                $"  >1.00     {counts[bins], 5}  (days cleaner than their month's mean turbidity)"
            );
    ...
    {
        Console.WriteLine(
            "=== Monthly means: both indices against the irradiation they come from ==="
        );
        Console.WriteLine(
            "month   clear-sky idx   classical Kt   observed   clear-sky   extraterr.   days"
        );

        foreach (var group in series.GroupBy(d => d.Date.Month).OrderBy(g => g.Key))
        {
            Console.WriteLine(
                $"{group.Key, 5} {group.Average(d => d.ClearSkyIndex), 14:F3} "
    ...
    private static void AcceptanceChecks(IReadOnlyList<DailyClearness> series)
    {
        ...
        foreach (var group in series.GroupBy(d => d.Date.Month).OrderBy(g => g.Key))
        {
            ...
            Console.WriteLine(
                $"{group.Key, 5} {over, 8} {100.0 * over / group.Count(), 11:F1} "
        ...
        foreach (var day in series.OrderBy(d => d.ClearSkyIndex).Take(5))
            Console.WriteLine(
                $"  low   {day.Date}  index {day.ClearSkyIndex:F3}  "
        ...
        foreach (var day in series.OrderByDescending(d => d.ClearnessIndex).Take(5))
            Console.WriteLine(
                $"  high  {day.Date}  classical Kt {day.ClearnessIndex:F3}  "
        ...
        Report(
            "Ceiling exceedance stays small",
            exceedanceRate < 0.10 && max < 1.25,
        ...
        Report(
            "Annual mean clear-sky index in 0.50-0.70",
            mean is >= 0.50 and <= 0.70,
            $"{mean:F3}"
        );
        ...
        Report(
            "Classical Kt behaves as literature expects",
        ...
        Report(
            "Index is far flatter seasonally than GHI",
            ktSpread < ghiSpread / 3.0,
        ...
        Console.WriteLine(
            $"  Deepest valley between two peaks: {separation:P0} below the lower peak."
        );
        Console.WriteLine(
            separation >= 0.25
        ...
        Console.WriteLine(
            "  Discrete states remain reasonable for capturing persistence, but their"
        );
        Console.WriteLine(
            "  boundaries are a modelling choice, not a feature the data hands over."
        );
