...
public static class ClearSkyCalibration
{
    ...
    public static void Run(IReadOnlyList<DwdSolarDay> days, DwdStation station)
    {
        ...
        Console.WriteLine(
            "  criteria: complete, sunshine ≥95% of possible, diffuse fraction <0.30"
        );
        ...
        Console.WriteLine(
            $"Usable clear-sky sub-samples: {samples.Count:N0} "
    ...
    {
        Console.WriteLine(
            "=== Daily-total accuracy on clear days (what Kt actually divides by) ==="
        );
        Console.WriteLine("month     Perez on bias   |err|      Perez off bias   |err|    days");

        double onBiasAll = 0.0,
            onAbsAll = 0.0,
            offBiasAll = 0.0,
            offAbsAll = 0.0;
        int dayCount = 0;
        ...
        foreach (var monthGroup in samples.GroupBy(s => s.Month).OrderBy(g => g.Key))
        {
            ...
            foreach (var dayGroup in monthGroup.GroupBy(s => s.Date))
            {
                ...
                double on = dayGroup.Sum(s =>
                    ModelledWhPerM2(s, withPerez[s.Month], station.AltitudeMeters, true)
                );
                double off = dayGroup.Sum(s =>
                    ModelledWhPerM2(s, withoutPerez[s.Month], station.AltitudeMeters, false)
                );
            ...
            Console.WriteLine(
                $"{monthGroup.Key, 5} {onBias / days, 15:P2} {onAbs / days, 8:P2} "
        ...
        Console.WriteLine();
        Console.WriteLine(
            $"Overall   Perez on : bias {onBiasAll / dayCount, 7:P2}, mean |error| {onAbsAll / dayCount:P2}"
        );
        Console.WriteLine(
            $"          Perez off: bias {offBiasAll / dayCount, 7:P2}, mean |error| {offAbsAll / dayCount:P2}"
        );
        Console.WriteLine();
    }
    ...
    private static List<DwdSolarDay> SelectClearDays(IReadOnlyList<DwdSolarDay> days) =>
        days.Where(d => d.IsComplete && d.SunshineFraction() >= 0.95 && d.DiffuseFraction < 0.30)
            .ToList();
    ...
    private static double ModelledWhPerM2(
        ClearSample sample,
        double turbidity,
        double altitude,
        bool perez
    ...
    )
    {
        ...
        for (int month = 1; month <= 12; month++)
        {
            ...
            for (
                double turbidity = MinTurbidity;
                turbidity <= MaxTurbidity;
                turbidity += TurbidityStep
            )
            {
                double error = 0.0;
                ...
                {
                    double residual =
                        ModelledWhPerM2(sample, turbidity, station.AltitudeMeters, perezEnhancement)
    ...
    private static void ReportTurbidity(double[] withPerez, double[] withoutPerez)
    {
        ...
        for (int month = 1; month <= 12; month++)
        {
            ...
            Console.WriteLine(
                $"{month, 5} {withPerez[month], 10:F2} {withoutPerez[month], 11:F2} "
    ...
    )
    {
        ...
        for (int month = 1; month <= 12; month++)
        {
            ...
            {
                Console.WriteLine(
                    $"{month, 5}        --          --  {low.Count, 8} {high.Count, 7}"
            ...
            double onTilt =
                MeanResidual(high, station, withPerez, true)
                - MeanResidual(low, station, withPerez, true);
            double offTilt =
                MeanResidual(high, station, withoutPerez, false)
                - MeanResidual(low, station, withoutPerez, false);

            Console.WriteLine(
                $"{month, 5} {onTilt, 10:P2} {offTilt, 11:P2} {low.Count, 8} {high.Count, 7}"
            );
        ...
        if (discriminatingMonths > 0)
        {
            ...
            Console.WriteLine();
            Console.WriteLine(
                "Smaller wins it means the model's air-mass dependence matches the sky's."
            );
        }
    ...
    )
    {
        ...
        foreach (var sample in samples)
        {
            sum +=
                (
                    ModelledWhPerM2(sample, turbidity[sample.Month], station.AltitudeMeters, perez)
                    - sample.MeasuredWhPerM2
                ) / sample.MeasuredWhPerM2;
        }
