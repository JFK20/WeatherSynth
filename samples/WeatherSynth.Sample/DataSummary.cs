...
public static class DataSummary
{
    ...
    )
    {
        ...
        Console.WriteLine(
            $"Missing global : {missingGlobal:N0} of {intervals.Count:N0} "
        ...
        for (int i = 1; i < days.Count; i++)
        {
            ...
            if (skipped > 0)
                Console.WriteLine(
                    $"  {days[i - 1].Date} -> {days[i].Date}  ({skipped} days absent)"
                );
        }
        Console.WriteLine();
        ...
        {
            Console.WriteLine(
                $"{group.Key, 5} {group.Count(), 5} {group.Average(d => d.GlobalWhPerM2) / 1000.0, 13:F2} "
        ...
        Console.WriteLine();
        Console.WriteLine(
            $"Annual GHI : {annualTotal:F0} kWh/m²/yr, month-weighted "
        ...
        foreach (double threshold in new[] { 0.95, 0.90, 0.80 })
        {
            ...
            Console.WriteLine(
                $"  sunshine ≥{threshold:P0}: {clear.Count, 4} days, "
