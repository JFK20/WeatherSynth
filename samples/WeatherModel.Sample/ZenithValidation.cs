...
public static class ZenithValidation
{
    public static void Run(IReadOnlyList<DwdSolarInterval> intervals, DwdStation station)
    {
        ...
        Console.WriteLine($"Station {station.Id} {station.Name}  " +
        ...
        Console.WriteLine("Mean bias by month (a seasonal swing means the equation of time is wrong):");
        for (int m = 1; m <= 12; m++)
            Console.WriteLine($"  {m,2}: {monthlyBias[m] / monthlyCount[m],+9:F5}°  (n={monthlyCount[m]:N0})");
        Console.WriteLine();

        Console.WriteLine("Mean bias by zenith band (growth towards the horizon means refraction):");
        for (int b = 0; b < 10; b++)
        {
            if (zenithCount[b] == 0) continue;
            Console.WriteLine($"  {b * 18,3}-{(b + 1) * 18,3}°: {zenithBias[b] / zenithCount[b],+9:F5}°  (n={zenithCount[b]:N0})");
        }
        Console.WriteLine();
    }
        ...
        foreach (double stepDegrees in new[] { 0.01, 0.002, 0.0005 })
        {
            ...
            var best = grid
                ...
                .Select(p => (p.Lat, p.Lon,
                              Rmse: Rmse(sample, station with { LatitudeDegrees = p.Lat, LongitudeDegrees = p.Lon })))
                .MinBy(r => r.Rmse);
            ...
            Console.WriteLine($"  step {stepDegrees:F4}° -> {bestLat:F4}°N {bestLon:F4}°E  RMSE {bestRmse:F5}°");
        }

        Console.WriteLine();

        double nominalRmse = Rmse(sample, station);
        Console.WriteLine($"Nominal  : {station.LatitudeDegrees:F4}°N {station.LongitudeDegrees:F4}°E  RMSE {nominalRmse:F5}°");
        Console.WriteLine($"Best fit : {bestLat:F4}°N {bestLon:F4}°E  RMSE {bestRmse:F5}°");
        Console.WriteLine($"Offset   : {bestLat - station.LatitudeDegrees:+0.0000;-0.0000}° lat, " +
    ...
    private static DailyClearSkyCalculator Build(DwdStation station) => new(
