...
public static class ClearSkySanity
{
    public static void Run(double latitude, double longitude, double altitude, string timeZoneId)
    {
        ...
        Console.WriteLine($"=== Site: {latitude:F4}°N {longitude:F4}°E, {altitude:F0} m, {timeZoneId} ===");
        Console.WriteLine($"Pressure at {altitude} m       : {ClearSkyIneichen.PressureFromAltitude(altitude):F0} Pa");
        Console.WriteLine($"Relative air mass at 90° zenith: {ClearSkyIneichen.RelativeAirMass(90):F2}");
        Console.WriteLine();
