...
public static class VisualizationExport
{
    ...
    public static int Run(IReadOnlyList<DwdSolarDay> days, DwdStation station)
    {
        ...
        {
            Console.Error.WriteLine($"Could not find viz/{TemplateFileName} in any parent directory.");
            return 1;
        }
        ...
        DwdStation station)
    {
        ...
        return new JsonObject
        {
            ...
            {
                ["observed"] = Round(AnnualMean(observed.Select(d => (d.Date, d.ObservedWhPerM2))), 1),
                ["synthetic"] = Round(AnnualMean(synthetic.Select(d => (d.Date, d.GhiWhPerM2))), 1)
            },
            ["months"] = MonthFits(observed, model),
            ["observed"] = Track(
                observed.Select(d => (d.Date, d.ClearSkyIndex, d.ClearSkyWhPerM2)), start),
            ["synthetic"] = Track(
                synthetic.Select(d => (d.Date, d.ClearSkyIndex, d.ClearSkyWhPerM2)), start)
        };
    }
    ...
    /// </summary>
    private static JsonArray MonthFits(IReadOnlyList<DailyClearness> observed, ClearSkyIndexModel model)
    {
    ...
    private static JsonObject Track(
        IEnumerable<(DateOnly Date, double Index, double Ceiling)> series, DateOnly start)
    {
