...
public static class ZenithDecomposition
{
    ...
    public static void Run(IReadOnlyList<DwdSolarInterval> intervals, DwdStation station)
    {
        ...
        {
            var solarTimes = new SolarTimes(interval.MidpointUtc, station.LatitudeDegrees, station.LongitudeDegrees);
            ...
            double dwdDeclination = ImpliedDeclinationDegrees(
                interval.ZenithDegrees, interval.HourAngleDegrees, phi);
        ...
        double totalDecl = 0.0, totalEot = 0.0;
        int totalCount = 0;

        for (int m = 1; m <= 12; m++)
        {
            if (countByMonth[m] == 0) continue;
        ...
        Console.WriteLine($"Mean equation-of-time  : {totalEot / totalCount:+0.000;-0.000} min " +
    ...
    {
        Console.WriteLine("=== Library declination through 2015-03-20 (equinox, fastest change) ===");

        foreach (int hour in new[] { 0, 6, 12, 18 })
        {
            var instant = new DateTimeOffset(2015, 3, 20, hour, 0, 0, TimeSpan.Zero);
            var solarTimes = new SolarTimes(instant, station.LatitudeDegrees, station.LongitudeDegrees);
            Console.WriteLine($"  {hour:00}:00 UTC : {(double)solarTimes.SolarDeclination.Radians * RadToDeg,+9:F4}°");
        }
    ...
    /// </summary>
    private static double ImpliedDeclinationDegrees(double zenithDegrees, double hourAngleDegrees, double phiRadians)
    {
