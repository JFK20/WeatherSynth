...
public static class ZenithImpact
{
    public static void Run(IReadOnlyList<DwdSolarDay> days, DwdStation station)
    {
        ...
        var calculator = new DailyClearSkyCalculator(
            station.LatitudeDegrees, station.LongitudeDegrees, station.AltitudeMeters, TimeZoneInfo.Utc);
        ...
        foreach (var day in days.Where(d => d.IsComplete))
        {
            ...
            {
                double turbidity = LinkeTurbidity.Default(
                    interval.MidpointUtc.UtcDateTime);
                int dayOfYear = interval.MidpointUtc.DayOfYear;

                libraryWh += ClearSkyIneichen.Estimate(
                    calculator.ApparentZenithDegrees(interval.MidpointUtc),
                    dayOfYear, turbidity, station.AltitudeMeters).Ghi;
                ...
                double dwdApparentZenith =
                    90.0 - (dwdElevation + DailyClearSkyCalculator.RefractionCorrectionDegrees(dwdElevation));

                dwdWh += ClearSkyIneichen.Estimate(
                    dwdApparentZenith, dayOfYear, turbidity, station.AltitudeMeters).Ghi;
            }

            if (dwdWh <= 0.0) continue;
        ...
        double totalSigned = 0.0, totalAbsolute = 0.0;
        int totalCount = 0;

        for (int m = 1; m <= 12; m++)
        {
            if (monthlyCount[m] == 0) continue;
            Console.WriteLine($"{m,5} {monthlyRelative[m] / monthlyCount[m],13:P3} " +
