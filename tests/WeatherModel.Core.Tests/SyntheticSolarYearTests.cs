...
public class SyntheticSolarYearTests
{
    ...
    public void GenerateYear_DifferentSeedsGiveDifferentWeatherOnTheSameCeiling()
    {
        ...
        second.Days.Select(d => d.ClearSkyIndex)
            .Should().NotEqual(first.Days.Select(d => d.ClearSkyIndex));
        ...
        second.Days.Select(d => d.ClearSkyWhPerM2)
            .Should().Equal(first.Days.Select(d => d.ClearSkyWhPerM2));
    }
    ...
    public void ClearSkyFraction_IsEnergyWeightedAndExceedsTheDailyMean()
    {
        ...
        year.ClearSkyFraction.Should().BeApproximately(
            year.GhiKWhPerM2 / year.ClearSkyKWhPerM2, 1e-12);
