...
internal static class ClimateFixtures
{
    ...
    internal static ScaledBeta SeasonalShape(DateOnly date)
    {
        ...
        return new ScaledBeta(
            seasonal * 8.0,
            (1.0 - seasonal) * 8.0,
            ClearSkyIndexModel.DefaultSupport
        );
    }
    ...
    private static readonly Lazy<ClearSkyIndexModel> LazySeasonalModel = new(() =>
        ClearSkyIndexModel.Fit(SeasonalSeries())
    );
